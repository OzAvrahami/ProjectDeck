...
export const PROJECT_LIFECYCLE_OPTIONS = PROJECT_LIFECYCLE_STATES.map(
  (value) => ({
    value,
...
export function validateProjectEdit(input) {
  ...
  const values = {
    ...
    lifecycleState: cleanText(input.lifecycleState),
    needsAttention,
  ...
  if (!PROJECT_LIFECYCLE_STATES.includes(values.lifecycleState)) {
    errors.lifecycleState = "Choose a valid lifecycle state.";
  }
