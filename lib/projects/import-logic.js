...
export function validateGitHubImportGroups(groups) {
  ...
  return groups.map((group) => {
    const targetProjectId = cleanOptionalText(group.targetProjectId);
    const projectName = cleanOptionalText(group.projectName);
    ...
    if (!Array.isArray(group.repositories) || group.repositories.length === 0) {
      throw new ImportValidationError(
        "Every import group needs at least one repository.",
      );
    }

    const groupedRepositories = group.repositories.map((repository) => {
    ...
    return {
      targetProjectId,
      projectName,
      nextAction: cleanOptionalText(group.nextAction),
