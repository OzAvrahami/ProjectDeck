...
function composePortfolioProjects(
  projectRows,
  componentRows,
  resourceRows,
  monitorRows,
...
export async function listPortfolioProjects() {
  ...
  return composePortfolioProjects(
    projectRows,
    componentRows,
    resourceRows,
    monitorRows,
...
export async function getProjectWorkspaceBySlug(slug) {
  ...
  const [projectRows, componentRows, resourceRows, monitorRows, associationRows] = await database.batch([
    ...
    database
      ...
      .where(inArray(resourceMonitors.projectId, projectIds))
      .orderBy(asc(resourceMonitors.label)),
    database
      .select()
      .from(providerResourceAssociations)
      .where(inArray(providerResourceAssociations.projectId, projectIds))
      .orderBy(asc(providerResourceAssociations.displayName)),
  ]);
...
export async function updateProjectOwnedFields(projectId, input) {
  const [project] = await getDatabase()
    .update(projects)
    .set({
      healthAlertsEnabled: input.healthAlertsEnabled ?? false,
      name: input.name,
      tagline: input.tagline,
      phaseOverride: input.phaseOverride ?? null,
