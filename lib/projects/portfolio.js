...
export function formatCompactRelativeTime(value, now = new Date()) {
  const timestamp = validTimestamp(value);
  ...
  if (elapsedSeconds < 2_592_000) return `${Math.floor(elapsedSeconds / 604_800)}w ago`;
  if (elapsedSeconds < 31_536_000) return `${Math.floor(elapsedSeconds / 2_592_000)}mo ago`;
  return `${Math.floor(elapsedSeconds / 31_536_000)}y ago`;
}

export function buildLatestCommitPresentation(
  activity,
  repositoryCount,
  now = new Date(),
) {
  ...
  const commit = activity.latestCommit ?? activity.items?.[0] ?? null;
  ...
  if (!commit) {
    return {
      ...
      visible: true,
      subject:
        state === "partial" ? "No verified commit activity" : "No commit activity",
      relativeLabel: state === "partial" ? "partial" : null,
      commit: null,
    };
  }

  const repositoryDisplayName =
    commit.repositoryDisplayName ??
    commit.repository?.name ??
    commit.repository?.fullName?.split("/").at(-1) ??
  ...
  return {
    ...
    relativeLabel:
      state === "partial"
        ? "partial"
        : formatCompactRelativeTime(commit.committedAt, now),
    scopeLabel:
      repositoryCount > 1
        ? commit.component?.name ?? repositoryDisplayName
        : null,
    commit: {
      sha: commit.sha,
      subject: commit.subject ?? commit.message,
      ...
      component: commit.component ?? null,
      url: commit.url,
    },
  };
}
...
export function buildProjectCardViewModel(project, now = new Date()) {
  ...
  const releaseSummary = releaseLabel
    ? {
        ...
        externalRelease: project.githubSummary.releases.hasSingleSafeProjectRelease
          && project.githubSummary.releases.state === "exact"
          ? project.githubSummary.releases.items[0]
          : null,
    ...
    : null;
  const latestCommit = buildLatestCommitPresentation(
    project.githubSummary?.activity,
    project.githubSummary?.repositoryCount ?? 0,
    now,
  );
