...
function resolutionUnknown(resolution) {
  if (!resolution || resolution.status === "unavailable") {
    return unknown("GitHub Project data is unavailable", {
      providerError: resolution?.error?.code ?? "provider",
    });
  }
...
export function inferProjectPhase(
  ...
  if (!readModel.priorityField?.standard) {
    return unknown("GitHub Project Priority field is nonstandard", {
      githubProject: {
        id: readModel.id,
        title: readModel.title,
      },
      priorityOptions: readModel.priorityField?.options ?? [],
    });
  }
