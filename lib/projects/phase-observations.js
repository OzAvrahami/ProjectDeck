...
import { attachProjectAttention } from "./attention.js";
...
export async function observeProjectWorkflowEvidence(
  projects,
  {
    ...
    fetchImpl = fetch,
    includeManualProjects = false,
  } = {},
) {
  const evidenceByProjectId = new Map();
  const projectsNeedingInference = includeManualProjects
    ? projects
    : projects.filter((project) => !project.nextAction?.trim());
...
export async function observeProjectsWithAutomation(
  ...
  const projectsWithAttention = attachProjectAttention(
    attachProjectNextActions(projectsWithPhase, workflowEvidence),
    workflowEvidence,
  );

  return projectsWithAttention.map((project) => ({
    ...project,
