...
export async function observeProjectWorkflowEvidence(
  ...
  const projectsNeedingInference = projects.filter(
    (project) => !project.phaseOverride || !project.nextAction?.trim(),
  );
...
export async function observeProjectsWithAutomation(
  ...
  const [observedProjects, workflowEvidence] = await Promise.all([
  ...
  const projectsWithPhase = attachProjectPhases(
    observedProjects,
    workflowEvidence,
    { now },
  );
