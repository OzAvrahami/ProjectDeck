export function hasCanonicalBugLabel(issue) {
  return (issue?.labels ?? []).some((label) => {
    const name = typeof label === "string" ? label : label?.name;
    return typeof name === "string" && name.trim().toLowerCase() === "bug";
  });
}

function isOpenIssue(issue) {
  return (
    issue?.type !== "pull_request" &&
    !issue?.pullRequest &&
    String(issue?.state ?? "").toLowerCase() === "open"
  );
}

function issueIdentity(issue) {
  if (issue?.id != null) {
    return `github:${issue.id}`;
  }

  const repository = issue?.repository?.fullName?.toLowerCase();
  return repository && issue?.number != null
    ? `github:${repository}#${issue.number}`
    : null;
}

function uniqueOpenIssues(items) {
  const unique = new Map();

  for (const issue of items) {
    if (!isOpenIssue(issue)) continue;
    const identity = issueIdentity(issue);

    if (identity && !unique.has(identity)) {
      unique.set(identity, issue);
    }
  }

  return [...unique.values()];
}

export function formatProjectIssueCounts({
  status,
  openIssueCount,
  openBugCount,
}) {
  if (status === "not_connected") {
    return { label: null, bugLabel: null, openLabel: null };
  }

  if (status === "unavailable") {
    return {
      label: "Issues unavailable",
      bugLabel: null,
      openLabel: null,
    };
  }

  const partial = status === "partial";
  const issueCount = partial ? `${openIssueCount}+` : String(openIssueCount);
  const bugCount = partial ? `${openBugCount}+` : String(openBugCount);
  const openLabel = `${issueCount} open`;

  if (openBugCount === 0 && !partial) {
    return { label: openLabel, bugLabel: null, openLabel };
  }

  const bugLabel = `${bugCount} ${openBugCount === 1 && !partial ? "bug" : "bugs"}`;
  return {
    label: `${bugLabel} · ${openLabel}`,
    bugLabel,
    openLabel,
  };
}

export function filterProjectIssues(items, type = "all") {
  return type === "bug"
    ? items.filter(hasCanonicalBugLabel)
    : items;
}

function observationStatus(successCount, failureCount, totalCount) {
  if (totalCount === 0) {
    return "not_connected";
  }

  if (successCount === totalCount) {
    return "complete";
  }

  return successCount > 0 && failureCount > 0 ? "partial" : "unavailable";
}

function newestFirst(field) {
  return (left, right) => {
    const leftTime = left[field] ? Date.parse(left[field]) : 0;
    const rightTime = right[field] ? Date.parse(right[field]) : 0;
    return (Number.isNaN(rightTime) ? 0 : rightTime) -
      (Number.isNaN(leftTime) ? 0 : leftTime);
  };
}

function withProjectContext(item, project, observation) {
  return {
    ...item,
    repository: item.repository ?? observation.repository,
    project: {
      id: project.id,
      slug: project.slug,
      name: project.name,
      accent: project.accent,
    },
    component: observation.componentId
      ? {
          id: observation.componentId,
          name: observation.componentName,
        }
      : null,
    scopeLabel: observation.scopeLabel,
  };
}

function releaseSummaryState(status) {
  return {
    complete: "exact",
    partial: "partial",
    unavailable: "unavailable",
    not_connected: "not_connected",
  }[status] ?? "unavailable";
}

function releaseScopeLabel(repository) {
  return (
    repository.component?.name ??
    repository.repository?.fullName ??
    repository.scopeLabel ??
    "Repository"
  );
}

function releaseCardLabel(release) {
  return `${release.tagName}${release.prerelease ? " · pre-release" : ""}`;
}

export function summarizeProjectReleases(project, projectObservations) {
  const releaseObservations = projectObservations.filter(
    (observation) => observation.release.status !== "not_requested",
  );
  const releaseSuccesses = releaseObservations.filter(
    (observation) => observation.release.status === "success",
  );
  const releaseFailures = releaseObservations.filter(
    (observation) => observation.release.status === "unavailable",
  );
  const status = observationStatus(
    releaseSuccesses.length,
    releaseFailures.length,
    releaseObservations.length,
  );
  const repositories = releaseObservations.map((observation) => ({
    resourceId: observation.resourceId,
    repository: observation.repository,
    component: observation.componentId
      ? {
          id: observation.componentId,
          name: observation.componentName,
        }
      : null,
    scopeLabel:
      observation.componentName ??
      observation.repository?.fullName ??
      observation.scopeLabel,
    providerStatus:
      observation.release.status === "success" ? "success" : "unavailable",
    latestRelease:
      observation.release.status === "success" && observation.release.item
        ? withProjectContext(observation.release.item, project, observation)
        : null,
    error:
      observation.release.status === "unavailable"
        ? observation.release.error
        : null,
    checkedAt: observation.checkedAt,
  }));
  const items = repositories
    .map((repository) => repository.latestRelease)
    .filter(Boolean)
    .sort(newestFirst("publishedAt"));
  const releaseBearingRepositoryCount = items.length;
  const hasSingleSafeProjectRelease =
    status === "complete" &&
    repositories.length === 1 &&
    releaseBearingRepositoryCount === 1;
  let safeCardLabel = null;

  if (status === "unavailable") {
    safeCardLabel = "Release unavailable";
  } else if (status === "partial") {
    safeCardLabel = "Release information incomplete";
  } else if (status === "complete" && repositories.length === 1) {
    safeCardLabel = items[0] ? releaseCardLabel(items[0]) : "No published Release";
  } else if (status === "complete" && repositories.length > 1) {
    if (releaseBearingRepositoryCount === 0) {
      safeCardLabel = "No published Release";
    } else if (releaseBearingRepositoryCount === 1) {
      const repository = repositories.find(({ latestRelease }) => latestRelease);
      safeCardLabel = `${releaseScopeLabel(repository)} · ${releaseCardLabel(repository.latestRelease)}`;
    } else {
      const allComponentScoped = repositories
        .filter(({ latestRelease }) => latestRelease)
        .every(({ component }) => component);
      safeCardLabel = `${releaseBearingRepositoryCount} ${allComponentScoped ? "component" : "repository"} releases`;
    }
  }

  return {
    status,
    state: releaseSummaryState(status),
    repositories,
    items,
    checkedRepositoryCount: releaseSuccesses.length,
    failedRepositoryCount: releaseFailures.length,
    releaseBearingRepositoryCount,
    safeCardLabel,
    hasSingleSafeProjectRelease,
    failures: releaseFailures.map(
      (observation) => observation.release.error,
    ),
  };
}

function activitySummaryState(status) {
  return {
    complete: "exact",
    partial: "partial",
    unavailable: "unavailable",
    not_connected: "not_connected",
  }[status] ?? "unavailable";
}

function commitIdentity(commit) {
  const repository = commit?.repository?.fullName?.toLowerCase();
  return repository && commit?.sha
    ? `github:${repository}@${commit.sha}`
    : null;
}

function uniqueCommits(items) {
  const unique = new Map();

  for (const commit of items) {
    const identity = commitIdentity(commit);
    if (identity && !unique.has(identity)) unique.set(identity, commit);
  }

  return [...unique.values()];
}

export function summarizeProjectActivity(project, projectObservations) {
  const activityObservations = projectObservations.filter(
    (observation) =>
      observation.activity && observation.activity.status !== "not_requested",
  );
  const activitySuccesses = activityObservations.filter(
    (observation) => observation.activity.status === "success",
  );
  const activityFailures = activityObservations.filter(
    (observation) => observation.activity.status === "unavailable",
  );
  const status = observationStatus(
    activitySuccesses.length,
    activityFailures.length,
    activityObservations.length,
  );
  const repositories = activityObservations.map((observation) => {
    const items = observation.activity.status === "success"
      ? observation.activity.items
          .map((activity) => withProjectContext(activity, project, observation))
          .sort(newestFirst("committedAt"))
      : [];

    return {
      resourceId: observation.resourceId,
      repository: observation.repository,
      repositoryDisplayName:
        observation.repository?.name ?? observation.repository?.fullName ?? null,
      component: observation.componentId
        ? {
            id: observation.componentId,
            name: observation.componentName,
          }
        : null,
      scopeLabel:
        observation.componentName ??
        observation.repository?.name ??
        observation.repository?.fullName ??
        observation.scopeLabel,
      providerStatus:
        observation.activity.status === "success" ? "success" : "unavailable",
      latestCommit: items[0] ?? null,
      error:
        observation.activity.status === "unavailable"
          ? observation.activity.error
          : null,
      checkedAt: observation.checkedAt,
      items,
    };
  });
  const items = uniqueCommits(
    repositories.flatMap((repository) => repository.items),
  ).sort(newestFirst("committedAt"));

  return {
    status,
    state: activitySummaryState(status),
    latestCommit: items[0] ?? null,
    items,
    repositories,
    checkedRepositoryCount: activitySuccesses.length,
    failedRepositoryCount: activityFailures.length,
    failures: activityFailures.map(
      (observation) => observation.activity.error,
    ),
  };
}

export function summarizeProjectGitHub(project, observations) {
  const projectObservations = observations.filter(
    (observation) => observation.projectId === project.id,
  );
  const issueObservations = projectObservations.filter(
    (observation) => observation.issues.status !== "not_requested",
  );
  const issueSuccesses = issueObservations.filter(
    (observation) => observation.issues.status === "success",
  );
  const issueFailures = issueObservations.filter(
    (observation) => observation.issues.status === "unavailable",
  );
  const issueItems = uniqueOpenIssues(issueSuccesses
    .flatMap((observation) =>
      observation.issues.items.map((issue) =>
        withProjectContext(issue, project, observation),
      ),
    ))
    .sort(newestFirst("updatedAt"));
  const issueStatus = observationStatus(
    issueSuccesses.length,
    issueFailures.length,
    issueObservations.length,
  );
  const hasProviderCounts = issueSuccesses.every(
    (observation) =>
      Number.isInteger(observation.issues.totalCount) &&
      Number.isInteger(observation.issues.bugCount),
  );
  const openIssueCount = hasProviderCounts
    ? issueSuccesses.reduce(
        (count, observation) => count + observation.issues.totalCount,
        0,
      )
    : issueItems.length;
  const openBugCount = hasProviderCounts
    ? issueSuccesses.reduce(
        (count, observation) => count + observation.issues.bugCount,
        0,
      )
    : issueItems.filter(hasCanonicalBugLabel).length;
  const filteredIssueCount = issueSuccesses.every((observation) =>
    Number.isInteger(observation.issues.filteredTotalCount),
  )
    ? issueSuccesses.reduce(
        (count, observation) => count + observation.issues.filteredTotalCount,
        0,
      )
    : issueItems.length;
  const issueCounts = formatProjectIssueCounts({
    status: issueStatus,
    openIssueCount,
    openBugCount,
  });

  const releaseSummary = summarizeProjectReleases(project, projectObservations);

  const activitySummary = summarizeProjectActivity(project, projectObservations);
  const implementationObservations = projectObservations.filter(
    (observation) =>
      observation.implementation &&
      observation.implementation.status !== "not_requested",
  );
  const implementationSuccesses = implementationObservations.filter(
    (observation) => observation.implementation.status === "success",
  );
  const implementationFailures = implementationObservations.filter(
    (observation) => observation.implementation.status === "unavailable",
  );
  const implementationStatus = observationStatus(
    implementationSuccesses.length,
    implementationFailures.length,
    implementationObservations.length,
  );
  const implementationRepositories = implementationSuccesses.map(
    (observation) => ({
      resourceId: observation.resourceId,
      repository: observation.repository,
      component: observation.componentId
        ? {
            id: observation.componentId,
            name: observation.componentName,
          }
        : null,
      scopeLabel: observation.scopeLabel,
      maturity: observation.implementation.maturity,
      activity: observation.implementation.activity,
      checkedAt: observation.checkedAt,
    }),
  );

  return {
    repositoryCount: projectObservations.length,
    checkedAt:
      projectObservations
        .map((observation) => observation.checkedAt)
        .filter(Boolean)
        .sort()
        .at(-1) ?? null,
    issues: {
      status: issueStatus,
      count: openIssueCount,
      openIssueCount,
      openBugCount,
      filteredIssueCount,
      label: issueCounts.label,
      bugLabel: issueCounts.bugLabel,
      openLabel: issueCounts.openLabel,
      items: issueItems,
      checkedRepositoryCount: issueSuccesses.length,
      failedRepositoryCount: issueFailures.length,
      failures: issueFailures.map((observation) => observation.issues.error),
    },
    releases: {
      ...releaseSummary,
      compactLabel: releaseSummary.safeCardLabel,
    },
    activity: activitySummary,
    implementation: {
      status: implementationStatus,
      repositories: implementationRepositories,
      checkedRepositoryCount: implementationSuccesses.length,
      failedRepositoryCount: implementationFailures.length,
      failures: implementationFailures.map(
        (observation) => observation.implementation.error,
      ),
    },
  };
}

export function attachGitHubSummaries(projects, observations) {
  return projects.map((project) => ({
    ...project,
    githubSummary: summarizeProjectGitHub(project, observations),
  }));
}

export function listCrossProjectIssues(projects) {
  return projects
    .flatMap((project) => project.githubSummary?.issues.items ?? [])
    .sort(newestFirst("updatedAt"));
}

export function listCrossProjectReleases(projects) {
  return projects
    .flatMap((project) => project.githubSummary?.releases.items ?? [])
    .sort(newestFirst("publishedAt"));
}

export function listCrossProjectActivity(projects) {
  return projects
    .flatMap((project) => project.githubSummary?.activity.items ?? [])
    .sort(newestFirst("committedAt"));
}

export function summarizeCrossProjectChecks(projects, feature) {
  const summaries = projects
    .map((project) => project.githubSummary?.[feature])
    .filter(Boolean);

  return {
    repositoryCount: summaries.reduce(
      (count, summary) =>
        count + summary.checkedRepositoryCount + summary.failedRepositoryCount,
      0,
    ),
    checkedRepositoryCount: summaries.reduce(
      (count, summary) => count + summary.checkedRepositoryCount,
      0,
    ),
    failedRepositoryCount: summaries.reduce(
      (count, summary) => count + summary.failedRepositoryCount,
      0,
    ),
    failures: summaries.flatMap((summary) => summary.failures),
  };
}

export function summarizeCrossProjectIssueCounts(projects) {
  const summaries = projects
    .map((project) => project.githubSummary?.issues)
    .filter(Boolean);
  const repositoryCount = summaries.reduce(
    (count, summary) =>
      count + summary.checkedRepositoryCount + summary.failedRepositoryCount,
    0,
  );
  const checkedRepositoryCount = summaries.reduce(
    (count, summary) => count + summary.checkedRepositoryCount,
    0,
  );
  const failedRepositoryCount = summaries.reduce(
    (count, summary) => count + summary.failedRepositoryCount,
    0,
  );
  const status = observationStatus(
    checkedRepositoryCount,
    failedRepositoryCount,
    repositoryCount,
  );
  const openIssueCount = summaries.reduce(
    (count, summary) => count + summary.openIssueCount,
    0,
  );
  const openBugCount = summaries.reduce(
    (count, summary) => count + summary.openBugCount,
    0,
  );

  return {
    status,
    openIssueCount,
    openBugCount,
    ...formatProjectIssueCounts({ status, openIssueCount, openBugCount }),
  };
}
