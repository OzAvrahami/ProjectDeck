export function buildIntegrationStatus({
  ...
  const vercelResourceCount = resources.filter(
    (resource) => resource.provider === "vercel",
  ).length;
  ...
  return {
    ...
    vercel: {
      configured: Boolean(vercelConfigured),
      connectedCount: connectionsAvailable ? vercelResourceCount : null,
    },
