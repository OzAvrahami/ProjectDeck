...
const DEPLOYMENT_STATE_QUERY = `
  query DeploymentState(
    $recentInput: DeploymentListInput!
    $activeInput: DeploymentListInput!
    $first: Int!
  ) {
    recent: deployments(first: $first, input: $recentInput) {
      edges {
        node {
    ...
    active: deployments(first: 1, input: $activeInput) {
      edges {
        node {
          id projectId environmentId serviceId status createdAt updatedAt statusUpdatedAt url
        }
...
export async function fetchRailwayDeploymentState(
  ...
  const data = await railwayGraphQL(
    DEPLOYMENT_STATE_QUERY,
    {
      recentInput: baseInput,
      activeInput: { ...baseInput, status: { successfulOnly: true } },
      first: Math.min(Math.max(Number(limit) || 10, 1), 20),
    },
    options,
  );
  return {
    deployments: (data.recent?.edges ?? [])
      .map(({ node }) => normalizeRailwayDeployment(node))
      .filter(Boolean),
    activeDeployment: normalizeRailwayDeployment(
      data.active?.edges?.[0]?.node ?? null,
    ),
  };
}
