...
export async function fetchGitHubGraphQL(
  ...
  if (!payload?.data || (payload.errors?.length ?? 0) > 0) {
    const messages = (payload?.errors ?? [])
      .map((error) => error?.message)
      .filter(Boolean)
      .join(" ");
    const code = /permission|scope|forbidden|not authorized/i.test(messages)
      ? "permission"
      : "provider";
    ...
    throw new GitHubProviderError(
      ...
      code === "permission"
        ? `The configured GitHub token does not allow ${capability}.`
        : `GitHub returned an incomplete response for ${capability}.`,
      { status: response.status },
    );
  }
