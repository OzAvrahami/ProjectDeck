...
export async function fetchGitHubProjectReadModel(project, options = {}) {
  ...
  const inaccessibleItemCount = rawItems.filter((item) => !item.content).length;
  ...
  return {
    ...
    partial:
      ...
      Boolean(projectNode.fields?.pageInfo?.hasNextPage) ||
      Boolean(projectNode.views?.pageInfo?.hasNextPage) ||
      Boolean(projectNode.workflows?.pageInfo?.hasNextPage),
    inaccessibleItemCount,
    statusField: {
      available: Boolean(statusField),
      standard: optionsMatch(statusField, STANDARD_PROJECT_STATUSES),
      id: statusField?.id ?? null,
    ...
    priorityField: {
      ...
      id: priorityField?.id ?? null,
      options: priorityField?.options.map(({ name }) => name) ?? [],
    },
    fields: normalizedFields,
    fieldsPartial: Boolean(projectNode.fields?.pageInfo?.hasNextPage),
    views: (projectNode.views?.nodes ?? []).filter(Boolean).map((view) => ({
      id: view.id,
      number: view.number,
      name: view.name,
      layout: view.layout,
      filter: view.filter ?? null,
      groupByFields: (view.groupByFields?.nodes ?? [])
        .filter(Boolean)
        .map((field) => ({ id: field.id ?? null, name: field.name ?? null })),
      verticalGroupByFields: (view.verticalGroupByFields?.nodes ?? [])
        .filter(Boolean)
        .map((field) => ({ id: field.id ?? null, name: field.name ?? null })),
    })),
    viewsPartial: Boolean(projectNode.views?.pageInfo?.hasNextPage),
    workflows: (projectNode.workflows?.nodes ?? [])
      .filter(Boolean)
      .map((workflow) => ({
        id: workflow.id,
    ...
    workflowsPartial: Boolean(projectNode.workflows?.pageInfo?.hasNextPage),
