...
Issue detail surfaces load bounded provider pages rather than draining every open Issue before rendering. Page navigation and the canonical Bug filter are deep-linkable, while the displayed open and Bug totals continue to describe the complete accessible repository set. Multi-repository and provider-partial evidence remains explicit.
...
Portfolio cards use a fixed information hierarchy: Project identity and description, Phase and Health, conditional Needs Attention, Next, compact Release and Issue context, then Latest Commit. A restrained monogram is the identity fallback until ProjectDeck has an explicit persisted logo source. Empty and unavailable states stay truthful but compact, while Component or repository scope appears only when required to interpret multi-repository evidence.

Navigation prioritizes orientation over secondary provider latency. The application shell, saved Project identity, and Workspace tabs should begin rendering within a roughly 250 ms warm-navigation budget where the local database permits it; provider-backed sections then resolve without replacing pending evidence with guesses. A Workspace tab requests only the provider feature families it presents, while Overview retains the complete Phase, Next, Health, attention, and GitHub Standard dependency graph.
