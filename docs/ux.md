...
Other monitoring configuration stays scoped to the relevant Project Workspace. The user explicitly chooses Vercel deployment, HTTP endpoint, or PostgreSQL and whether the monitor affects Project Health. Existing generic Resources are not pinged automatically. GitHub setup remains reachable through Projects and Settings, while the Railway provider connection remains a restrained integration surface rather than an infrastructure console.
