...
The card's underlined `Releases →` label opens the Project's Workspace Releases tab. An exact single-repository published Release also shows its concise tag as an underlined external link with an arrow, opening the authoritative GitHub Release in a new tab. Pre-release markers remain visible. Empty and uncertain values are plain text: `No published Release`, `Release unavailable`, or `Release information incomplete`; the internal Releases route remains available. One released repository in a multi-repository Product is scope-prefixed; multiple released Components use a compact count instead of a synthetic Product version. Card links remain independent siblings of the full-card Workspace link. Recorded Component application versions are metadata, not published Releases.

Workspace Overview's Releases rail explicitly links to the Releases tab. That tab emphasizes `Latest Release`, the exact tag, a meaningful Release name, publication date, and `Open on GitHub ↗`. Multi-repository Products identify each Component/repository before its version and retain successful absence and unavailable evidence per repository. Repository identity remains secondary for a single repository. `Release history on GitHub ↗` opens the repository's history without copying Release bodies or loading more provider evidence. Global Releases rows retain Project, Component/repository, tag, publication date, and an explicit `Open Release on GitHub ↗` action; the Project name opens its internal Releases surface. External actions identify GitHub and new-tab navigation accessibly, and links have persistent underlines and visible keyboard focus.
